...
void playGamepvp(Card deck[], int deckSize) {
    Card player1Hand[54], player2Hand[54];
    int player1HandSize = 0, player2HandSize = 0;
    Card topCard;
    int currentPlayer = 0; // 0 for player, 1 for computer
    bool gameOver = false;
    ...
    // Deal initial cards (7 cards to each player)
    for (int i = 0; i < 7; i++) {
        drawCard(deck, &deckSize, player1Hand, &player1HandSize);
        drawCard(deck, &deckSize, player2Hand, &player2HandSize);
    }
    ...
    while (!gameOver) {
        if (currentPlayer == 0) {
            // Player1's turn
            printf("\nYour turn!\n");
            displayHand(player1Hand, player1HandSize, "Player1");
            ...
            if (choice == 0) {
                ...
                drawCard(deck, &deckSize, player1Hand, &player1HandSize);
                printf("You drew a card.\n");
            } else if (choice > 0 && choice <= player1HandSize) {
                // Check if the chosen card is valid
                Card chosenCard = player1Hand[choice - 1];
                if (isValidMove(topCard, chosenCard)) {
                    // Play the card
                    topCard = chosenCard;
                    printf("You played: %s %s\n", topCard.color, topCard.value);
                    ...
                    // Remove the card from the player's hand
                    for (int i = choice - 1; i < player1HandSize - 1; i++) {
            ...
            // Player2's turn
            printf("\nYour turn!\n");
            displayHand(player2Hand, player2HandSize, "Player2");
            ...
            if (choice == 0) {
                ...
                drawCard(deck, &deckSize, player1Hand, &player1HandSize);
                printf("You drew a card.\n");
            } else if (choice > 0 && choice <= player2HandSize) {
                // Check if the chosen card is valid
                Card chosenCard = player2Hand[choice - 1];
                if (isValidMove(topCard, chosenCard)) {
                    // Play the card
                    topCard = chosenCard;
                    printf("You played: %s %s\n", topCard.color, topCard.value);
                    ...
                    // Remove the card from the player's hand
                    for (int i = choice - 1; i < player2HandSize - 1; i++) {
                        player2Hand[i] = player2Hand[i + 1];
                    }
                    player2HandSize--;
